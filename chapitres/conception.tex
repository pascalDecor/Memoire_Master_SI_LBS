\chapter[Conception de la solution]{Conception de la solution d'archivage intelligent}
\label{ch:conception}

Au \nomMinistere{}, l'analyse menée au chapitre précédent a mis en évidence les limites d'un traitement documentaire encore largement manuel. Identifier l'expéditeur, renseigner les informations du document, déterminer son emplacement et le retrouver parmi un volume croissant d'archives mobilisent un temps important et rendent le traitement plus difficile à mesure que les documents s'accumulent. Ces constats ont permis de dégager plusieurs besoins : automatiser les tâches répétitives sans retirer aux agents leur rôle de contrôle, fiabiliser le classement, faciliter la recherche et contribuer à la sécurité ainsi qu'à la traçabilité des opérations.
...
\begin{table}[htbp]
\centering
\caption{Correspondance entre les besoins du chapitre~\ref{ch:existant} et les choix de conception}
...
\toprule
\rowcolor{black!12}
\tableheader{Besoin (chapitre~\ref{ch:existant})} &
...
\begin{figure}[H]
\centering
\input{images/fig_modele_donnees_conception}
\caption{Diagramme de classes UML : modèle documentaire et recherche}
\label{fig:modele-donnees-conception}
\end{figure}

Le contrôle des accès et la traçabilité relèvent du même modèle, mais portent sur les acteurs plutôt que sur le contenu. Un utilisateur est rattaché à des rôles, eux-mêmes associés à des permissions, et peut appartenir à des groupes ; les événements journalisent une action, un horodatage et l'utilisateur concerné, en lien avec le document le cas échéant. La figure~\ref{fig:modele-donnees-securite} isole cette dimension. Le document y apparaît de manière réduite : il sert de point d'accroche avec le modèle documentaire, sans en répéter les attributs métier. Les listes de contrôle d'accès portent les droits fins sur les nœuds. Le principal d'une règle n'est pas un type de chaîne redondant avec des associations parallèles : \texttt{Utilisateur}, \texttt{Groupe} et \texttt{Role} spécialisent une classe abstraite \texttt{Principal}, et \texttt{ControleAcces} n'y est lié qu'une fois.

\begin{figure}[H]
\centering
\input{images/fig_modele_donnees_securite}
...
Le \textbf{groupe} est une entité organisationnelle. Il rassemble des utilisateurs et peut porter des rôles, afin d'appliquer des droits homogènes à un ensemble d'agents. Comme l'utilisateur et le rôle, il spécialise \texttt{Principal} et peut donc être le sujet d'une règle d'accès.

Au-delà du RBAC, un \textbf{contrôle d'accès} (\texttt{ControleAcces}) rattache un \texttt{Principal} à un nœud. La généralisation évite de dupliquer le lien vers l'utilisateur, le groupe et le rôle, et de compenser cette duplication par un attribut \texttt{typePrincipal}. La règle porte une permission (lecture, écriture, suppression, validation), un effet et une propagation éventuelle dans le sous-arbre. Le niveau de sensibilité, lui, reste un attribut du document. Les \textbf{événements} relient une action, un horodatage, un utilisateur, un document et éventuellement un nœud, afin de distinguer ce que le pipeline a proposé de ce qu'un agent a décidé.

\subsection{Contraintes importantes}

Plusieurs contraintes assurent la cohérence du modèle. Un nœud de type fichier ne référence qu'un seul document, et un document n'apparaît qu'une fois dans l'arborescence. L'index d'un segment est unique au sein d'un document, afin d'éviter des doublons d'indexation. Les statuts du nœud et la source du flux sont des valeurs fermées, pour que les règles de classement et de visibilité restent déterministes. L'empreinte SHA-256 du fichier est conservée avec le document : elle ne remplace pas un identifiant métier, mais permet la vérification d'intégrité et la détection de contenus identiques. La réalisation présentera ce mécanisme dans l'interface de vérification. Les référentiels ne peuvent pas être supprimés tant qu'ils restent utilisés par des documents. En cas de suppression physique d'un document, ses segments associés sont également supprimés. Une règle \texttt{ControleAcces} référence exactement un \texttt{Principal} ; le polymorphisme de cette association interdit qu'un même contrôle pointe à la fois vers un utilisateur et un groupe. Au niveau physique, cette généralisation pourra se traduire par une colonne discriminante, qui n'est plus un attribut métier du diagramme de classes. Enfin, les filtres de droits et de sensibilité s'appliquent aux requêtes comme à la récupération RAG, de sorte qu'aucune relation du modèle ne contourne le périmètre d'accès.
...
L'analyse de l'existant a montré que la recherche dépend encore trop souvent du parcours des dossiers ou de la mémoire des agents. Une fois les documents décrits et classés, la plateforme doit donc permettre de les retrouver autrement. L'indexation textuelle et vectorielle, puis une assistance fondée sur la génération augmentée par récupération (\textit{Retrieval-Augmented Generation}, RAG), répondent à ce besoin. Le RAG associe un modèle génératif à un mécanisme de récupération d'informations externes, afin de fournir au modèle un contexte documentaire adapté à la requête \citp{lewis2020}{Lewis et al., 2020}. Le pipeline d'archivage constitue le corpus ; le RAG l'interroge ensuite de manière ancrée.

\subsection{Architecture du pipeline RAG}

Le fonctionnement prévu enchaîne des responsabilités distinctes (figure~\ref{fig:rag-pipeline}). L'utilisateur pose une question depuis l'interface ; le système applique d'abord ses droits, puis récupère des passages, les fusionne, en réduit la taille et les fournit à un modèle de langage pour produire une réponse accompagnée de ses sources \citp{lewis2020}{Lewis et al., 2020}. L'objectif n'est pas d'obtenir une réponse « générique », mais une réponse rattachée aux archives du ministère et traçable jusqu'aux documents mobilisés.
...
Pour qu'une question formulée en langage naturel puisse rejoindre le bon passage, le contenu doit être découpé et représenté de façon adaptée. On distingue deux objets d'indexation. Les \textbf{segments de contenu} sont des passages d'environ $n_{c}$ caractères, avec un chevauchement $o$ destiné à préserver le contexte local. Les \textbf{champs métier} (objet, expéditeur, type documentaire, destinataire, numéro interne) sont indexés à part : ce ne sont pas des chunks de contenu, mais des attributs structurés vectorisés afin qu'une requête portant sur un organisme ou un numéro puisse les atteindre directement.

Chaque unité ainsi indexée --- segment de contenu ou champ métier --- reçoit une représentation vectorielle stockée en base. Ces représentations denses permettent une \textbf{récupération dense} : rapprocher l'embedding de la requête de ceux des unités indexées, au-delà de la seule correspondance de mots-clés (\citer{reimers2019}{Reimers et Gurevych, 2019} ; \citer{karpukhin2020}{Karpukhin et al., 2020}). Le modèle d'embedding peut être local ou cloud selon l'environnement : ce qui change alors, c'est le calcul des vecteurs et éventuellement la réindexation ; le reste du dispositif (récupération, filtres de sécurité, génération) demeure stable. Cette souplesse compte dans un contexte ministériel où les contraintes de souveraineté peuvent évoluer.

La figure~\ref{fig:indexation-vectorielle} résume ce dispositif : le document est découpé en champs métier et en segments de contenu, puis chaque unité est vectorisée et conservée dans l'index.

\begin{figure}[H]
\centering
\resizebox{0.92\textwidth}{!}{\input{images/fig_indexation_vectorielle}}
\caption{Indexation vectorielle : champs métier et segments de contenu, puis embedding}
\label{fig:indexation-vectorielle}
\end{figure}

\subsection{Récupération hybride}

Les observations réalisées dans le cadre de l'étude montrent que les agents ne formulent pas toujours leurs besoins avec les mêmes mots que le document original. Une recherche uniquement dense, ou uniquement par mots-clés, resterait donc partielle.

Le raisonnement retenu place la sécurité \textbf{avant} le classement, et non après que le modèle de langage ait vu des extraits : corpus $C$ (nœuds \texttt{VALIDATED} par défaut), puis filtrage de sécurité, puis trois stratégies de récupération, puis fusion RRF, diversification et sélection des $N$ extraits. Soit $D \subseteq C$ l'ensemble des documents autorisés à être récupérés, après application des ACL, du plafond de sensibilité et du statut. Les trois stratégies opèrent directement sur $D$, de sorte qu'aucun contenu hors périmètre n'entre dans le ranking.

On note $\mathcal{M} = \{\mathrm{dense}, \mathrm{lex}, \mathrm{doc}\}$ l'ensemble des méthodes, et $L_{m}$ la liste de candidats produite par la méthode $m \in \mathcal{M}$ :
\begin{itemize}
  \item $L_{\mathrm{dense}}$ : récupération dense au sens du Dense Passage Retrieval, par similarité entre l'embedding de la requête et ceux des unités indexées (segments de contenu et champs métier) \citp{karpukhin2020}{Karpukhin et al., 2020} ;
  \item $L_{\mathrm{lex}}$ : correspondances lexicales dans le texte des segments de contenu ;
  \item $L_{\mathrm{doc}}$ : recherche lexicale au niveau document, reposant à la fois sur l'indexation plein texte et sur les champs de métadonnées.
\end{itemize}
Ces listes ne partagent pas la même échelle de scores ; les fusionner par une somme de similarités n'aurait pas de sens. On les combine par Reciprocal Rank Fusion \citp{cormack2009}{Cormack, Clarke et Buettcher, 2009}, qui ne retient que le rang $r_{m}(d)$ de $d$ dans $L_{m}$.

Pour tout $d \in D$,
\begin{equation}
\label{eq:rrf}
\mathrm{RRF}(d) = \sum_{m \in \mathcal{M}} \frac{1}{k + r_{m}(d)}.
\end{equation}
Le rang $r_{m}(d)$ vaut $1$ pour le premier résultat de $L_{m}$. Si $d$ n'apparaît pas dans $L_{m}$, le terme correspondant est omis (contribution nulle). La constante de lissage $k$ atténue l'influence des têtes de liste. Elle est fixée à $k = 60$, configuration classique reprise de \citer{cormack2009}{Cormack, Clarke et Buettcher (2009)} ; cette valeur n'a pas été optimisée sur le jeu de test du chapitre~\ref{ch:realisation}. Aucune pondération supplémentaire n'est appliquée aux branches : $w_{m} = 1$ pour tout $m \in \mathcal{M}$. Un réordonnancement lexical léger n'intervient qu'après cette fusion, et n'entre pas dans l'équation~\eqref{eq:rrf}.

Les paramètres de conception du RAG et de l'indexation sont posés dans le tableau~\ref{tab:params-rag-conception}. Les seuils de similarité restent configurables. Les valeurs du tableau sont celles retenues pour la conception et confirmées à la réalisation ; elles ne prétendent pas à un optimum expérimental pour ce corpus.

\begin{table}[htbp]
\centering
\caption{Paramètres de conception de l'indexation, de la fusion RRF et du RAG}
\label{tab:params-rag-conception}
\small
\renewcommand{\arraystretch}{1.40}
\begingroup
\rowcolors{2}{black!3}{white}
\begin{tabularx}{\textwidth}{@{}L{4.3cm} L{1.8cm} Y@{}}
\toprule
\rowcolor{black!12}
...
\tableheader{Valeur} \\
\midrule
Taille d'un segment de contenu & $n_{c}$ & $\approx$\,2000 caractères \\
Chevauchement entre segments & $o$ & 400 caractères \\
Constante de lissage RRF & $k$ & 60 (littérature, non calibrée) \\
Pondération des méthodes $\mathcal{M}$ & $w_{m}$ & $1$ (équipondération) \\
Nombre d'extraits transmis au modèle & $N$ & 8 \\
Champs métier exploités & --- & objet, expéditeur, type documentaire, destinataire, numéro interne \\
Corpus RAG par défaut & $C$ & nœuds \texttt{VALIDATED} (hors \texttt{DRAFT} et corbeille) \\
\bottomrule
\end{tabularx}
\endgroup
\end{table}

La procédure nominale s'enchaîne ainsi : (1)~filtrer $C$ en $D$ selon les droits, la sensibilité et le statut ; (2)~construire $L_{\mathrm{dense}}$, $L_{\mathrm{lex}}$ et $L_{\mathrm{doc}}$ sur $D$ ; (3)~calculer $\mathrm{RRF}(d)$ selon l'équation~\eqref{eq:rrf} ; (4)~diversifier (un même document ne monopolise pas les $N$ extraits) et retenir $N$ passages ; (5)~réduire chaque passage par une sélection extractive des phrases les plus proches de la requête, puis en borner la longueur, afin de limiter les informations redondantes transmises au modèle. Cette réduction n'est pas une méthode nommée de \textit{context compression} ; elle vise surtout à contenir la taille du prompt, conformément au constat que les modèles exploitent mal un contexte trop long \citp{liu2024}{Liu et al., 2024}.

Deux mécanismes complémentaires n'entrent dans cette chaîne que lorsque la première récupération ne fournit pas d'extraits suffisants : une expansion lexicale de la requête par synonymes du vocabulaire métier \citp{azad2019}{Azad et Deepak, 2019}, et une seconde passe (seuils assouplis, éventuellement reformulation). Ils sont prévus dans l'architecture et implémentés, mais n'ont pas été isolés dans l'évaluation du chapitre~\ref{ch:realisation}. Cette combinaison vise ainsi à réduire le risque de manquer un document lorsque la formulation de la requête diffère de celle employée dans l'archive.
...
\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{\input{images/fig_architecture_generale}}
\vspace{0.35em}
{\footnotesize\textbf{Légende :} bleu = services, vert = infrastructure, gris = accès et supervision.
Les traitements longs passent par RabbitMQ ; l'interface (7) reste indépendante.\par}
\caption{Architecture générale de la plateforme en déploiement sur site}
...
\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{\input{images/fig_communication_async}}
