...
\node[infra] (vault) at (2, -3.0) {Vault\\(secrets)};
\node[infra] (kc)    at (6, -3.0) {Keycloak\\(auth M2M)};
\node[obs]   (obs)   at (11,-3.0) {Observabilité\\Prometheus • Grafana\\Loki • Vector};
...
\draw[dottedline] (kc.north) -- (route.south);
\draw[dottedline] (vault.north) -- (pg.south);
\draw[dottedline] (obs.north) -- (redis.south);
\end{tikzpicture}%
}
\vspace{0.2em}
\footnotesize \textbf{Légende :} bleu = microservices, vert = infrastructure (dont Vault et Keycloak), pointillé = liens transverses (sécurité / supervision).
