...
\node[infra] (pg)    at (1.95, 3.70) {PostgreSQL + pgvector\\[-0.4pt]{\scriptsize métadonnées, RBAC, index}};
\node[infra] (minio) at (5.70, 3.70) {MinIO\\[-0.4pt]{\scriptsize stockage objets}};
\node[infra] (redis) at (9.45, 3.70) {Redis\\[-0.4pt]{\scriptsize cache / état}};
...
\endgroup
\vspace{0.15em}
\footnotesize \textbf{Légende :} bleu = services, vert = infrastructure, gris = accès et supervision. Les traitements longs passent par RabbitMQ ; l'interface (7) reste indépendante.
