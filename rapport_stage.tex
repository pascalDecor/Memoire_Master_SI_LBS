...
Enfin, ce rapport est structuré en trois parties. La première présente le cadre de la mission (structure d’accueil, contexte, existant et analyse des besoins). La deuxième détaille la conception et la réalisation de la solution (architecture, modèle de données, implémentation et déploiement). La troisième aborde les aspects d’exploitation, de sécurité et d’évaluation, avant de conclure sur le bilan du projet et les perspectives d’évolution.
...
Compte tenu de la sensibilité des informations traitées et des exigences de souveraineté, la solution devait être déployée en environnement local (\textit{on-premise}). Elle devait également s’intégrer aux pratiques existantes des services, caractérisées par :
\begin{itemize}
...
\begin{itemize}
  \item RAGAS (\textit{Retrieval-Augmented Generation Assessment}) : cadre d’évaluation des systèmes RAG.
  \item PostgreSQL et extension pgvector : indexation vectorielle et recherche sémantique.
  \item Keycloak : gestion des identités et des accès (IAM) / SSO.
  \item HashiCorp Vault : gestion sécurisée des secrets.
\end{itemize}
