...
Un document scanné n'est pas encore une archive. Au \nomMinistere{}, le traitement documentaire reposait encore largement sur des opérations manuelles, sur la séparation entre un serveur de fichiers et un index Excel, et sur une recherche trop dépendante du parcours des dossiers. Le fonds est principalement constitué de documents papier numérisés et de PDF, auxquels s'ajoutent d'autres formats produits par l'activité administrative.

Ce mémoire présente la conception et la réalisation d'une plateforme d'archivage numérique déployée en environnement interne. La chaîne assiste l'acquisition, la reconnaissance du contenu, la proposition de métadonnées, le renommage et le classement. Un agent valide ou corrige ces propositions avant toute intégration définitive au corpus. La recherche hybride et un assistant conversationnel permettent d'interroger les documents à partir de leur contenu, sous contrainte des droits d'accès.

L'intelligence artificielle n'y est pas une autorité de décision. Elle prépare une description et un classement ; la responsabilité reste humaine. L'architecture modulaire permet de faire évoluer un moteur d'OCR, un modèle d'embeddings ou un modèle de langage sans reconstruire l'ensemble du système.

L'évaluation menée sur le fonds de production (35\,007 documents ; taux d'échec moyen d'environ 5~\% en exploitation) et sur un échantillon de test de 205 documents montre que la chaîne tient à l'échelle interne. La recherche ramène le document pertinent dans les cinq premiers résultats pour 62~\% des questions du test (MRR de 0,597). Ces chiffres décrivent un fonds distinct du flux annuel d'environ 10\,000 dossiers et du fonds historique. Ils ne permettent pas d'affirmer une précision métier de l'OCR, de la classification ou de l'extraction : le gold du harnais recopie encore les sorties de production. La qualité des scans, un référentiel relu par les agents et la qualité des réponses générées restent des facteurs à approfondir.

\noindent\textbf{Mots-clés :} archivage numérique ; intelligence artificielle ; GED ; OCR ; RAG ; administration publique.

\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract}
A scanned document is not yet an archive. At the \nomMinistere{}, document processing still relied largely on manual operations, on the separation between a file server and an Excel index, and on retrieval that depended too heavily on browsing folders. The holdings consist mainly of digitized paper documents and PDFs, together with other formats produced by administrative activity.

This thesis presents the design and implementation of a digital archiving platform deployed in an internal environment. The pipeline supports acquisition, content recognition, metadata suggestion, renaming and classification. An officer validates or corrects these suggestions before any document is permanently added to the corpus. Hybrid search and a conversational assistant make it possible to query documents from their content, subject to access-control constraints.

Artificial intelligence is not treated as a decision-making authority. It prepares a description and a filing proposal; responsibility remains with the human officer. The modular architecture allows an OCR engine, an embedding model or a language model to be replaced without rebuilding the whole system.

Evaluation on the production holdings (35,007 documents; average operational failure rate of about 5\%) and on a test sample of 205 documents shows that the pipeline holds at internal scale. Search returns the relevant document in the top five results for 62\% of the test questions (MRR of 0.597). These figures describe holdings distinct from the annual flow of about 10,000 files and from the historical fonds. They do not support claims of OCR, classification or extraction accuracy: the evaluation gold still copies production outputs. Scan quality, a human-reviewed reference set and the quality of generated answers remain to be investigated further.

\noindent\textbf{Keywords:} digital archiving; artificial intelligence; EDM; OCR; RAG; public administration.
...
L'étude de l'existant a montré que le traitement documentaire reposait encore largement sur des opérations manuelles, sur la séparation entre les fichiers stockés et leur indexation, ainsi que sur un parcours des dossiers peu adapté à une recherche fondée sur le contenu. Ces contraintes devenaient d'autant plus importantes que les volumes documentaires augmentaient et que les sources de documents se diversifiaient.

Pour répondre à ces difficultés, une plateforme d'archivage numérique a été conçue et réalisée. La solution développée assiste une partie de la chaîne de traitement, depuis l'acquisition du document, principalement un scan ou un PDF, jusqu'à l'extraction de son contenu, la proposition de métadonnées, le classement et l'indexation. D'autres formats peuvent entrer dans la même chaîne. Elle intègre également une recherche hybride et un assistant permettant d'interroger le fonds à partir du contenu des documents. L'architecture modulaire retenue permet de faire évoluer les moteurs d'OCR, les modèles d'embeddings et les modèles de langage sans remettre en cause l'ensemble du système.

L'un des choix essentiels du projet a été de ne pas considérer l'intelligence artificielle comme une autorité de décision. Les propositions produites par les modèles restent soumises à une validation humaine avant leur intégration définitive au corpus documentaire. Cette approche permet de bénéficier de l'automatisation tout en maintenant la responsabilité des agents sur les décisions sensibles. Les mécanismes de contrôle d'accès, de gestion de la sensibilité, de journalisation et de filtrage des informations avant leur transmission à l'assistant complètent ce principe d'assistance contrôlée.

La réalisation et l'exploitation de la solution montrent que cette approche est opérationnelle à l'échelle du fonds de production : 35\,007 documents traités, un taux d'échec moyen d'environ 5~\% en exploitation, un rappel au rang~5 de 62~\% sur le split de test. La recherche par contenu et l'assistance conversationnelle permettent d'aller au-delà du simple parcours de l'arborescence. Toutefois, les scores d'exactitude obtenus contre les sorties de production ne mesurent pas la qualité métier de l'OCR, de la classification ou de l'extraction. Un gold relu, ainsi que des mesures de génération (ROUGE-L, taux d'ancrage), restent nécessaires avant d'affirmer un niveau de performance généralisable.
...
Enfin, les perspectives développées au chapitre~\ref{ch:bilan} visent d'abord à amplifier le gain de temps pour les secrétaires et les agents d'archivage. Un module de connecteurs, le Model Context Protocol et des alertes intelligentes sont envisagés comme leviers de cette orientation ; ils ne font pas partie de la solution réalisée. Ces évolutions devront rester soumises aux mêmes exigences de sécurité, de contrôle des accès et de supervision humaine.

Au terme de ce travail, la solution réalisée constitue ainsi une première réponse concrète à la problématique posée. Elle ne cherche pas à supprimer l'intervention humaine, mais à la repositionner sur les tâches où son jugement apporte le plus de valeur. La plateforme fournit ainsi un cadre permettant de rendre le traitement documentaire plus structuré, plus accessible et davantage assisté par l'intelligence artificielle, tout en conservant les exigences de contrôle nécessaires à la gestion des archives administratives.
